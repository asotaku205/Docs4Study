...
const getAllUsers = async (req) => {
  ...
  if (req.query.isActive !== undefined) {
    ...
    if (isActive === undefined) throw createServiceError("Invalid isActive value", 400, { message: "Invalid isActive value" });
...
const getUserById = async (req) => {
  ...
  if (!user) throw createServiceError("User not found", 404, { message: "User not found" });
  return user;
};
...
const deleteUser = async (req) => {
  ...
  if (!deletedUser) throw createServiceError("User not found", 404, { message: "User not found" });
...
const restoreUser = async (req) => {
  ...
  if (!restoredUser) throw createServiceError("User not found", 404, { message: "User not found" });
